                ...
                Real rin, Real min_rho_q, std::string b_field_type)
{
    ...
    ParArrayND<Real> A("A", pmb->ncells1, pmb->ncells2);
    // TODO figure out how much of this needs to be double instead of Real.  Any?
    pmb->par_for("B_field_A", 0, pmb->ncells2-1, 0, pmb->ncells1-1,
        KOKKOS_LAMBDA_2D {
            GReal Xembed[NDIM];
    ...
    pmb->par_for("B_field_B", 0, pmb->ncells3-2, 0, pmb->ncells2-2, 0, pmb->ncells1-2,
        KOKKOS_LAMBDA_3D {
            ...
            // Flux-ct
            P(prims::B1, k, j, i) = -(A(i, j) - A(i, j + 1) + A(i + 1, j) - A(i + 1, j + 1)) /
                                (2. * pmb->pcoord->dx2v(j) * G.gdet(Loci::center, j, i));
            P(prims::B2, k, j, i) = (A(i, j) + A(i, j + 1) - A(i + 1, j) - A(i + 1, j + 1)) /
...
Real GetLocalBetaMin(MeshBlock *pmb, EOS *eos, Grid G, GridVars P)
{
...
void NormalizeBField(MeshBlock *pmb, Grid G, GridVars P, Real factor)
{
