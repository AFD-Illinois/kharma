...
KOKKOS_INLINE_FUNCTION void SetConnectionCoeffFromMetricDerivs(
    const Real dg[NDFULL][NDFULL][NDFULL], Real Gamma[NDFULL][NDFULL][NDFULL])
{
...
KOKKOS_INLINE_FUNCTION void SetConnectionCoeffByFD(
    const System& s, Real Gamma[NDFULL][NDFULL][NDFULL], Args... args)
{
...
KOKKOS_INLINE_FUNCTION void SetGradLnAlphaByFD(
    const System& s, Real dx, Real X0, Real X1, Real X2, Real X3, Real da[NDFULL])
{
...
void Lower(
    const double Vcon[NDFULL], const double Gcov[NDFULL][NDFULL], double Vcov[NDFULL])
{
    SPACETIMELOOP(mu)
    {
        Vcov[mu] = 0.;
...
KOKKOS_INLINE_FUNCTION
void Raise(
    const double Vcov[NDFULL], const double Gcon[NDFULL][NDFULL], double Vcon[NDFULL])
{
    SPACETIMELOOP(mu)
    {
        Vcon[mu] = 0.;
...
KOKKOS_INLINE_FUNCTION void SetMetricGradientByFD(const System& s, Real dx, Real X0,
    Real X1, Real X2, Real X3, Real dg[NDFULL][NDFULL][NDFULL])
{
    Real EPS = std::pow(std::numeric_limits<Real>::epsilon(), 1.0 / 3.0);
...
void SetGeometryDefault(Data* rc, const System& system)
{
    ...
    auto lamb = KOKKOS_LAMBDA(
        const int b, const int k, const int j, const int i, CellLocation loc)
    {
        Real C[NDFULL];
    ...
    parthenon::par_for(DEFAULT_LOOP_PATTERN, "SetGeometry::Set Cached data, Corn",
        DevExecSpace(), 0, pack.GetDim(5) - 1, kb.s, kb.e + 1, jb.s, jb.e + 1, ib.s,
        ib.e + 1,
      KOKKOS_LAMBDA(const int b, const int k, const int j, const int i)
        {
