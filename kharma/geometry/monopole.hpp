...
class MonopoleSph
{
  public:
    ...
    void SpacetimeMetric(Real X0, Real X1, Real X2, Real X3, Real g[NDFULL][NDFULL]) const
    {
        ...
        const Real a = MonopoleGR::Interpolate(
            r, hypersurface_, rgrid_, MonopoleGR::Hypersurface::A);
        const Real a2 = a * a;
        ...
        Real X0, Real X1, Real X2, Real X3, Real dg[NDFULL][NDFULL][NDFULL]) const
    {
        ...
        const Real dalphadr = MonopoleGR::Interpolate(
            r, gradients_, rgrid_, MonopoleGR::Gradients::DALPHADR);
        const Real dbetadr = MonopoleGR::Interpolate(
            r, gradients_, rgrid_, MonopoleGR::Gradients::DBETADR);
        const Real dadt =
            MonopoleGR::Interpolate(r, gradients_, rgrid_, MonopoleGR::Gradients::DADT);
        const Real dalphadt = MonopoleGR::Interpolate(
            r, gradients_, rgrid_, MonopoleGR::Gradients::DALPHADT);
        const Real dbetadt = MonopoleGR::Interpolate(
            r, gradients_, rgrid_, MonopoleGR::Gradients::DBETADT);

        LinearAlgebra::SetZero(dg, NDFULL, NDFULL, NDFULL);
        // d/dt
        dg[0][0][0] = 2 * a * beta * beta * dadt - 2 * alpha * dalphadt +
                      2 * a * a * beta * dbetadt;
