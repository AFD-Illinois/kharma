...
void Initialize<FMKSMeshBlock>(ParameterInput* pin, StateDescriptor* geometry)
{
    ...
    // Require x0 to be <= x1 of the innermost ghost cell face to avoid issues with
    // mapping theta outside of [0, pi] This calculation assumes no mesh refinement, but
    // works for any refinement because ghost cell layers of refined blocks will always
    // have smaller extents
