...
class BoyerLindquist
{
  public:
    ...
    BoyerLindquist(Real a)
        : dx_(1e-8)
        , a_(a)
        , a2_(a * a)
    {}
    KOKKOS_INLINE_FUNCTION
    BoyerLindquist(Real a, Real dx)
        : dx_(dx)
        , a_(a)
        , a2_(a * a)
    {}
        ...
        Real X0, Real X1, Real X2, Real X3, Real g[NDFULL][NDFULL]) const
    {
        ...
        g[0][3] = g[3][0] = -ratio(a_ * r3dm * sth2,
        ...
        Real X0, Real X1, Real X2, Real X3, Real dg[NDFULL][NDFULL][NDFULL]) const
    {
        ...
        dg[3][3][1] =
            ratio(2 * r2 * (a2_ - rm2 * rm2 * r) -
                      2 * a2_ * (a2_ + r2 * (2 * r - 3)) * cth2 - 2 * a4 * (r - 1) * cth4,
                denom2 * sth2);
        ...
        const Real num =
            -a2pr2 * cth *
            (3 * a4 + 8 * a2_ * r2 + 8 * rm2 * r3 + 4 * a2_ * (a2_ + 2 * rm2 * r) * c2th +
                a4 * std::cos(4 * th)) *
            sth;
  ...
  private:
    KOKKOS_INLINE_FUNCTION
    void ComputeInternals_(
        const Real r, const Real th, Real& bl_fact, Real& cth2, Real& sth2) const
    {
        cth2 = std::cos(th);
        cth2 *= cth2;
        sth2 = 1.0 - cth2;
    ...
    KOKKOS_INLINE_FUNCTION
    void ComputeDeltaMu_(Real r, Real th, Real& r2, Real& r3, Real& sth, Real& cth,
        Real& DD, Real& mu) const
    {
        using namespace Utils;
        using namespace robust;
        sth = std::sin(th);
...
using CBoyerLindquistMesh = CachedOverMesh<Analytic<BoyerLindquist, IndexerMesh>>;
