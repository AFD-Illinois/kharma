...
class McKinneyGammieRyan
{
  public:
    ...
    McKinneyGammieRyan(bool derefine_poles, Real h, Real xt, Real alpha, Real x0,
        ...
        : derefine_poles_(derefine_poles)
        , h_(h)
        , xt_(xt)
        , alpha_(alpha)
        , x0_(x0)
    ...
    {}
    McKinneyGammieRyan(
        bool derefine_poles, Real h, Real xt, Real alpha, Real x0, Real smooth)
        : derefine_poles_(derefine_poles)
        , h_(h)
        , xt_(xt)
        , alpha_(alpha)
    ...
    void operator()(Real X1, Real X2, Real X3, Real C[NDSPACE],
        Real Jcov[NDSPACE][NDSPACE], Real Jcon[NDSPACE][NDSPACE]) const
    {
        using robust::ratio;
        Real y, th, thJ;
        ...
        const Real drdX1 =
            ...
            (1. + (hexp_dr > 0.) * hexp_csq_ * hexp_nsq_ *
                      std::pow(hexp_dr, hexp_nsq_ - 1.));
