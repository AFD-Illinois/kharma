    ...
    ParameterInput* pin, std::shared_ptr<Packages_t>& packages)
{
    ...
    std::string eos_type =
        pin->GetOrAddString(block_name, std::string("type"), "IdealGas");
    ...
    if (eos_type.compare(IdealGas::EosType()) == 0) {
        ...
        if (!pin->DoesParameterExist(block_name, "gamma") &&
            !pin->DoesParameterExist("GRMHD", "gamma")) {
            throw std::runtime_error("IdealGas EOS requires that gamma be specified in "
                                     "<eos> or <GRMHD> block!");
        }
        ...
        EOS eos_host = UnitSystem<IdealGas>(IdealGas(gm1, Cv),
            eos_units_init::length_time_units_init_tag, time_unit, mass_unit, length_unit,
            temp_unit);
        ...
        PARTHENON_REQUIRE_THROWS(
            use_ye, "\"StellarCollapse\" EOS requires that Ye be enabled!");
        needs_ye = use_ye;
        ...
        EOS eos_host =
            UnitSystem<StellarCollapse>(StellarCollapse(filename, use_sp5, filter_bmod),
                eos_units_init::length_time_units_init_tag, time_unit, mass_unit,
        ...
        for (int y = 0; y < n / 2; y++) {
            ...
            for (int r = 0; r < n; r++) {
                for (int t = 0; t < n; t++) {
                    eps = eos_sc.InternalEnergyFromDensityTemperature(rho, T, lambda) /
                          sie_unit;
                    P = eos_sc.PressureFromDensityTemperature(rho, T, lambda) /
                        press_unit;
