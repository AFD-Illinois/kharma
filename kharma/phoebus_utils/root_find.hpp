...
struct RootFind
{
    ...
    template<typename F>
    KOKKOS_INLINE_FUNCTION void refine_bracket(
        F& func, const Real guess, Real& a, Real& b, Real& ya, Real& yb)
    {
        if (a <= guess && b >= guess) {
            ya = func(guess);
            yb = func(b);
        ...
        const Real guess, RootFindStatus* status = nullptr)
    {
        ...
        if (status != nullptr) {
            *status = iteration_count == max_iter ? RootFindStatus::failure
                                                  : RootFindStatus::success;
        }
    ...
    template<typename F>
    KOKKOS_INLINE_FUNCTION Real bisect(
        F func, Real a, Real b, const Real tol, RootFindStatus* status = nullptr)
    {
