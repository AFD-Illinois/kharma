...
if __name__=='__main__':
	...
	for r, res in enumerate(RES):
		...
		if CONDUCTION:
			q_analytic   = np.loadtxt(os.path.join(kharmadir, 'prob/emhd/','conducting_atmosphere_{}_default'.format(res), 'atmosphere_soln_phi.txt'))[NG:-NG]
		...
		if CONDUCTION:
			L1_norm[r,3] = np.mean(np.fabs(q-q_analytic[:,None]))
